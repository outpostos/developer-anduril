...
- `task_id` — _string_ — ID of Task to get.

- `definition_version` — _uint_ — If set to a number greater than zero, retrieves specific definition_version, otherwise retrieves the latest definition_version.

- `task_view` — _enum_ — Optional - select which view of the task to fetch. If not set, defaults to TASK_VIEW_MANAGER.
...
- `task` — _object_ — The Task returned from Lattice.
