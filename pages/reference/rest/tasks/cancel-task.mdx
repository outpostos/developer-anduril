...
**端点**: `PUT /api/v1/tasks/:taskId/cancel`
...
通过在系统中标记任务待取消来取消任务。此方法会根据任务当前状态发起任务取消：- 如果任务尚未发送给 agent，则立即取消并将任务转换为终止状态（`STATUS_DONE_NOT_OK`，错误码为 `ERROR_CODE_CANCELLED`）。- 如果任务已发送给 agent，则取消请求会路由到该 agent。随后由 agent 决定是否可以取消：- 如果可以取消，agent 必须使用 `UpdateTaskStatus` 并将任务状态设为 `STATUS_DONE_NOT_OK`。- 如果无法取消，agent 必须使用 `UpdateTaskStatus` 将 `TaskError` 附加到任务上，错误码设为 `ERROR_CODE_REJECTED`，并在 `message` 中说明任务无法取消的原因。
...
- `taskId` — _string · required_ — 要取消的任务 ID

### 请求

此端点需要一个对象。

- `author` — _object · optional_ — 请求取消此任务的主体。
