...
使用指定参数在系统中创建新任务。此方法会启动一个带唯一 ID 的新任务（由调用方提供或自动生成），将初始任务状态设为 STATUS_CREATED，并建立任务所有权。可通过 Relations 字段将任务分配给特定 agent。创建后，任务进入生命周期工作流，并可通过其他 Tasks API 端点跟踪、更新和管理。
...
- `taskId` — _string · optional_ — 如果非空，则设置请求的任务 ID；否则生成新的随机 GUID。如果提供的任务 ID 不匹配 [A-Za-z0-9_-.]36，则拒绝。
...
- `relations` — _object · optional_ — 与此任务关联的任何关系，例如父任务或被指定执行此任务的 assignee。

- `isExecutedElsewhere` — _boolean · optional_ — 如果设置，则服务不会在 agent 上触发此任务执行。适用于从外部系统摄取任务，且该外部系统负责在 agent 上触发任务执行的场景。

- `initialEntities` — _list of objects · optional_ — 表示可用于执行实体感知任务的一组初始实体。例如实体 Objective、实体 Keep In Zone 等。

- `retryStrategy` — _object · optional_ — 任务执行或更新的任何重试策略。

- `deliveryConstraints` — _object · optional_ — Lattice 交付任务时的任何调度约束。

- `displayName` — _string · optional · deprecated_ — 此任务的人类可读显示名称，应较短（&lt;100 个字符）。
