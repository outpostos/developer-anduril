...
建立服务器流式连接，把任务交付给可任务化 agent 执行。此方法会从 Tasks API 到 agent 创建持久连接，使服务器可以在任务可用时将其推送给 agent。agent 会收到与其 selector 条件（实体 ID）匹配的任务流。该流交付三类请求：- ExecuteRequest：包含要由 agent 执行的新任务 - CancelRequest：表示应取消某个任务 - CompleteRequest：表示应完成某个任务。这是可任务化 agent 实时接收和处理任务的主要方法。agent 应保持此连接，并根据自身能力处理传入任务。当 agent 收到任务时，应使用 UpdateStatus 端点更新任务状态，将进度信息回传给 Tasks API。这是一个长轮询 API，会阻塞直到有新任务准备好交付。如果没有新任务可用，服务器会保持你的请求最多 5 分钟；5 分钟超时后，你需要重新发起新请求。
...
- `agentSelector` — _object · optional_ — 用于确定 agent 接收哪些 agent 任务的 selector 条件

### 响应

要求 agent 遵从的请求。

- `executeRequest` — _object · optional_ — 执行任务的请求。包含要执行任务的唯一 ID。

- `cancelRequest` — _object · optional_ — 取消任务的请求。包含该任务以及取消任务请求的 assignee。

- `completeRequest` — _object · optional_ — 完成任务的请求。包含要完成任务的唯一 ID。
